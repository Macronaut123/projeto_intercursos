package classes 
{
	 ...
	 */
	public class Local extends PropriedadesTela
	{
		...
		{		
			...
			iconArray = new Array("sanitario", "educacao", "esporte", "alimentacao", "cultura", "financeira", "taxi", "onibus", "metro", "posto", "estacionamento", "trem");
		...
		private function showFilterType(item:DisplayObject):void 
		{
			if (allowChange)
			{
				if (item.width < 1 && item.height < 1)
				{
					TweenLite.to(item, 0.5, { width:7.2, height: 16, onComplete: setChange,  ease:Back.easeOut  } );
				}
				else if (item.width > 0 && item.height > 0 )
				{
					TweenLite.to(item, 0.5, { width:0.0, height: 0.0, onComplete: setChange,  ease:Back.easeOut   } );
		...
		private function dispatch($eventType:MouseEvent):void
		{
